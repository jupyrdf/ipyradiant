...
class QueryWidget(W.VBox):
    ...
    def run_query(self, button):
        ...
        namespaces = NS_PATTERN.findall(
            self.query_constructor.namespaces
        )

        # Construct the query string
        res = self.graph.query(
            self.query_constructor.formatted_query.value,
            initNs=dict(namespaces)
        )
        collapsed_data = DataFrame(list(res))
        for ii, row in collapsed_data.iterrows():
            for jj, cell in enumerate(row):
                if type(cell) is URIRef:
