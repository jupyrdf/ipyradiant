...
class UnnamedVis(W.VBox):
    ...
    def validate_children(self, proposal):
        ...
        if not children:
            children = (
                W.HBox(
                    [self.type_selector, self.viewer],
                    layout=W.Layout(height="80%")
                ),
                self.json_output,
            )
        return children
    ...
    def update_graph(self, change):
        ...
        assert len(self.uri_to_string_type.keys()) <= len(COLOR_LIST), f"Cannot render more than {len(color_list)} visually disctinct colors."
        color_type_map = list(zip([*self.uri_to_string_type.values(), "multi-type"], COLOR_LIST_CSS))
